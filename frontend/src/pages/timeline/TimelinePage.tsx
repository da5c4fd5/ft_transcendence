...
import type { SharedMemory } from '../guest/guest.types';
...
export function TimelinePage({ onNavigateToToday, onPreviewGuest }: { onNavigateToToday?: () => void; onPreviewGuest?: (memory: SharedMemory) => void }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setYears(availableYears.length > 0 ? availableYears : [currentYear]);
  ...
  const handleDayClick = useCallback(async (date: string) => {
    ...
    if (entry) {
      try {
        ...
        return;
      } catch { /* ignore — entry not available */ }
    }
  }, []);
          ...
          onPreviewGuest={() => {
            if (selectedEntry) {
              onPreviewGuest?.({
                ...
                ownerName:           '',
                friendContributions: selectedEntry.friendContributions,
              });
            }
            setSelectedEntry(null);
          }}
